...
check_port() {
    local origin=$1 target=$2 port=$3 expected=$4 desc=$5
    sudo docker exec "$origin" nc -zv -w 2 "$target" "$port" > /dev/null 2>&1
    local status=$?
    if [ "$expected" = "true" ]; then
        [ $status -eq 0 ] && echo -e "  [${GREEN}OK${NC}] $desc (Port $port): ALLOWED" \
                          || echo -e "  [${RED}FAILURE${NC}] $desc (Port $port): IMPROPERLY BLOCKED"
    else
        [ $status -ne 0 ] && echo -e "  [${GREEN}OK${NC}] $desc (Port $port): BLOCKED" \
...
check_port "inet-client-01" "10.0.10.254" "8080" "true"  "Internet -> Snare (via FW)"
check_port "inet-client-01" "10.0.20.21"  "8090" "false" "Internet -> Tanner Backend (Forbidden)"
check_port "dmz-snare-01"   "10.0.20.21"  "8090" "true"  "Snare -> Tanner API"
...
[ $? -ne 0 ] && echo -e "${GREEN}BLOCKED (Correct)${NC}" || echo -e "${RED}ALLOWED (Incorrect)${NC}"
